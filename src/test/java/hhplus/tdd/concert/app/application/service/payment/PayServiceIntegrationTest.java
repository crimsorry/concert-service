...
public class PayServiceIntegrationTest {
    ...
    @Test
    public void 낙관락_retry_한_유저가_1건예약_100번결제시_1건결제성공() throws InterruptedException {
        long beforeTime = System.currentTimeMillis();
        ...
        IntStream.range(0, totalTasks).forEach(i ->
                executorService.execute(() -> {
                    try {
                        payService.processPayOptimisticLockRetry(testBase.waitingActive.getToken(), testBase.payment.getPayId());
                    } catch (RuntimeException e) {
                        failCnt.getAndIncrement();
                    } finally {
                        latch.countDown();
