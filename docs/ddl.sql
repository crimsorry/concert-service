-- FK 연관관계를 설정할 때, 개발자가 의도하지 않은 deadlock이나 cascade 관련 문제가 발생할 수 있습니다.
-- 이러한 이유로 이번 프로젝트에서는 JPA 엔터티에서 연관관계를 직접 관리하고,
-- DDL 작성 시 FK 제약 조건을 삭제하여 데이터베이스 제약 대신 JPA의 연관관계 관리 기능을 활용할 계획입니다.

-- index 사용:
-- 인덱스는 기존 외래 키(foreign key)와 상태 코드와 같이 조회가 자주 발생하는 컬럼에 적용했습니다.
-- 이를 통해 조회 쿼리의 성능 향상을 기대할 수 있습니다.
...
CREATE TABLE waiting (
    ...
    INDEX waiting_idx_status (status),
    INDEX waiting_idx_waiting_id (waiting_id)
) COMMENT = '대기열';
...
CREATE TABLE concert_schedule (
    ...
    INDEX schedult_idx_concert_id (concert_id)
) COMMENT = '콘서트 일정';
...
CREATE TABLE reservation (
    ...
    INDEX resrvation_idx_reserve_status (reserve_status),
    INDEX resrvation_idx_seat_id (seat_id)
) COMMENT = '예약 정보';
...
CREATE TABLE amount_history (
    ...
    INDEX amount_idx_member_id (member_id),
